...
| `d` | 聚焦到詳細資訊面板 |
| `F2` | 顯示地區建議（需先輸入地區關鍵字） |
| `F3` | 使用 AI 解析自然語言輸入（需先輸入自然語言） |
| `F4` | 開啟料理類別選擇器（45+ 種料理類型） |
| `↑` / `↓` | 在結果列表/建議選項中上下移動 |
| `Tab` | 在元件間切換 |
...
2. 按 `F4` 開啟料理類別選擇彈窗
3. 使用 `↑`/`↓` 方向鍵瀏覽 45+ 種料理類型
4. 按 `Enter` 確認選擇（或按 `Esc` 取消）
5. 選擇的料理名稱會自動填入關鍵字輸入框
6. 系統會自動使用料理類別代碼進行精確過濾
7. 繼續輸入地區並搜尋

**為什麼要使用料理類別選擇器？**
- ✅ **精確過濾**：只顯示該料理類型的專門店（例如搜尋すき焼き只顯示すき焼き專門店）
- ✅ 避免錯字：從列表選擇，確保料理名稱正確
- ✅ 探索選項：瀏覽所有支援的料理類型（和食、洋食、中華、焼肉、鍋、居酒屋等）
